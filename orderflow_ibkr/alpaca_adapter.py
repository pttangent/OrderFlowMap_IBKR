...
def _rfc3339_to_ns(value: str) -> int:
    ...
    if text.endswith("Z"):
        ...
        # Alpaca stock data is normally Zulu. Keep a safe ISO fallback for tests
        # and future endpoint variants that may include an explicit offset.
    ...
    if "." in body:
        ...
        whole, frac = body, ""
    dt = datetime.fromisoformat(whole).replace(tzinfo=offset)
    sec = int(dt.timestamp())
    nanos = int((frac[:9]).ljust(9, "0")) if frac else 0
    return sec * 1_000_000_000 + nanos


class AlpacaAdapter:
    """Read-only Alpaca stock market-data WebSocket adapter.

    The adapter emits the exact same QuoteEvent / TradeEvent contract used by
    IBKR so the OrderFlowEngine, SQLite writer, UI, and signals remain broker-
    agnostic. Entitlements are intentionally not guessed locally: IEX is the
    normal free live feed, while SIP requires the account's corresponding
    market-data entitlement. Alpaca's server remains the source of truth.
    """
    ...
    @staticmethod
    def _raise_for_errors(batch: list[dict], stage: str) -> None:
        for item in batch:
            if item.get("T") == "error":
                code = item.get("code")
                msg = item.get("msg") or item.get("message") or "unknown error"
                raise RuntimeError(f"Alpaca {stage} error {code}: {msg}")
    ...
    async def _open_and_subscribe(self) -> None:
        ...
        self._raise_for_errors(connected, "connect")
        ...
        self._raise_for_errors(auth, "auth")
        ...
        self._raise_for_errors(subscribed, "subscribe")
    ...
    def _handle_item(self, item: dict) -> None:
        ...
        if kind == "q":
            quote = QuoteEvent(
                ...
                bid_size=float(item["bs"]) if item.get("bs") is not None else None,
                ask_size=float(item["as"]) if item.get("as") is not None else None,
